...
class SpaceShip:
    ...
    def draw(self):
        ...
        pygame.draw.rect(display, self.colour, (self.x, self.y, self.w, self.h/2))
        pygame.draw.rect(display, background, (self.x + (self.w/3), self.y , self.w - (self.w * 2 / 3), self.h - (self.h / 3)))
...
# ------------------ Alien Class ---------------
class Alien:
    def __init__(self, x, y, d, color, speed):
        self.x = x
        self.y = y
        self.d = d
        self.x_dir = 1
...
def game():
    ...
    num_aliens = 8
    alienSize = 50
    bossAlien = Alien(alienSize, alienSize+20, alienSize, red, 4)
    bossKilled = False
    for i in range(num_aliens):
        newAlien = Alien((i+1)*alienSize + i*20, alienSize+20, alienSize, green, 3)
    ...
    while not invasion:
        ...
        if num_aliens <= 4 and not bossKilled:
            bossAlien.draw()
            bossAlien.move()
            for item in list(bullets):
                if item.hit(bossAlien.x, bossAlien.y, bossAlien.d):
                    bullets.remove(item)
                    num_bullet -= 1
                    bossKilled = True
            if bossAlien.x >= width:
                GameOver()
                pygame.display.update()
                time.sleep(3)
                invasion = True

        if num_aliens == 0 and bossKilled:
            saved()
            invasion = True
